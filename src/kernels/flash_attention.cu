...
//     const float
//         *__restrict__ k_cache, // [num_blocks, num_heads, PAGE_SIZE,
//         head_dim]
//     const float
//         *__restrict__ v_cache, // [num_blocks, num_heads, PAGE_SIZE,
//         head_dim]
//     const int *__restrict__ block_tables, // [num_seqs, max_blocks_per_seq]
//     const int *__restrict__ context_lens, // [num_seqs]
//     const int *__restrict__ q_lens, // [num_seqs]  <- 异构的核心！每个序列的
//     Q
//                                     // 长度不同
...
//     int lane_id =
//         threadIdx.x; // 用于在 head_dim (128) 上做并行规约，通常是一个 Warp
//         (32)
...
//     // head_dim] 排列
//     （实际工程中偏移量计算会更复杂，我们先用伪代码抽象概念） int num_heads =
//     blockDim.x; const float *my_q =
...
//         // 这个 kv_idx 到底存在哪个物理页 (Physical Block) 的哪个偏移
//         (Offset)？
//         // =====================================================
...
//         // =====================================================
//         // 用当前线程 (lane_id) 配合 Warp 级规约，计算 my_q 和 target_k
//         的点积。
...
//     for (int index = lane_id; index < context_len; index += 32) {
//         my_logits[index] = __expf(my_logits[index] - global_max) /
//         global_sum;
//     }
